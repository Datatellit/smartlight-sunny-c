...
{
  return( (!IS_LAMP_NODEID(gConfig.nodeID) && !IS_GROUP_NODEID(gConfig.nodeID)) || 
...
void InitNodeAddress() {
  ...
  if( !IS_LAMP_NODEID(gConfig.nodeID) && !IS_GROUP_NODEID(gConfig.nodeID) ) {
...
bool IsEntireConfigInvalid() {
  return( gConfig.version != XLA_VERSION 
