...
bool isMatch()
{
  if( isIdentityEmpty(gConfig.remoteUniqueid,UNIQUE_ID_LEN) )
  {
    return TRUE;
  }
  else
  {
    return isIdentityEqual(gConfig.remoteUniqueid,rcvMsg.header.uniqueid,UNIQUE_ID_LEN);
  }
}
...
uint8_t ParseProtocol(){
  ...
  bool _specificNode = ((rcvMsg.header.destination == gConfig.nodeID ) && IS_LAMP_NODEID(gConfig.nodeID));
  if(_cmd != C_INTERNAL || _type != I_CONFIG || _sensor != NCF_DEV_SET_REMOTE)
  {
#ifdef ENABLE_SDTM
    if(!isMatch())
    {
      ...
      if( _type == V_STATUS ) {
        ...
        else if( _lenPayl >= 3)
        {
          ...
          if(isGradual)
          {
            ...
            {
              ChangeBRByTime(0,DEVST_Bright,(int32_t)time * 60 * 200); //5ms timer
            }
          }
        }
...
{
  return !(!IS_LAMP_NODEID(nodeid) && !IS_GROUP_NODEID(nodeid));
}
//////set rf /////////////////////////////////////////////////
//typedef struct
//{
